...
public class ICEClientUtil {
	...
	private static ObjectPrx createIceProxy(Ice.Communicator communicator,
		...
		int pos = serviceName.lastIndexOf("PrxHelper");
		if (pos <= 0) {
		...
		try {
			...
			proxy = (ObjectPrx) Class.forName(clsName).newInstance();
	...
	public  static void init(String _iceLocator,long _idleTimeOutSeconds){
			iceLocator = _iceLocator;
